...
const CompanyPage = () => {
  ...
  return (
    <div>
    <div className="wholepage w-full h-full min-h-screen bg-[#f5f5dc] flex flex-col">
  {/* Categories Section at the Top */}
  <div className="bg-[#f5f5dc] p-4 shadow-md w-full">
    ...
    <ul className="grid grid-cols-6 gap-4 max-w-7xl mx-auto">
      {categories.map((catname, idx) => (
        <li key={idx} className="w-full">
          <button
            className="bg-[#87ceeb] rounded-xl shadow-md py-2 cursor-pointer flex flex-col items-center 
                       transition-all duration-300 hover:shadow-xl hover:scale-105 w-full"
            onClick={() => handleCategory(catname.name)}
          >
            <div className="flex">
              <p>{catname.icon}  </p>   
              <span className=" text-white">{catname.name.toLocaleUpperCase()}</span></div>
